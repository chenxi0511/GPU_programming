...
int main(int argc, char *argv[]) 
{
    ...
    if(total_pixels % total_thread == 0)
        numPixels_per_thread = total_pixels / total_thread;
    else
        numPixels_per_thread = total_pixels / total_thread + 1;

    if((width - 8) % 120 == 0)
        numBlcoks_col = (width - 8) / 120;
    else
        numBlcoks_col = (width - 8) / 120 + 1;

    if((height - 8) % 120 == 0)
        numBlocks_row = (height - 8) / 120;
    else
        numBlocks_row = (height - 8) / 120 + 1;

    int total_block = numBlocks_row * numBlcoks_col;
	int num_loops = (total_block % 12 == 0)? total_block / 12 : total_block / 12 + 1;
	
	if(partId == 'a')
	{
		imageFilterKernelPartA<<<grid, block>>>((char3*) d_inputPixels, (char3*) d_outputPixels, width, height , numPixels_per_thread);
	} 
	else if(partId == 'b')
	{
		imageFilterKernelPartB<<<grid, block>>>((char3*) d_inputPixels, (char3*) d_outputPixels, width, height , numPixels_per_thread, total_thread);
	}
	else if(partId == 'c')
	{
		imageFilterKernelPartC<<<grid, block>>>((char3*) d_inputPixels, (char3*) d_outputPixels, width, height, numBlocks_row, numBlcoks_col, num_loops);
	}
